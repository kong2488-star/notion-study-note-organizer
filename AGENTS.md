...
- `notion_auto_organizer/ai/__init__.py`: `ai` 서브패키지의 공개 API를 정의합니다.
- `notion_auto_organizer/ai/client.py`: 공통 `AIClient` 프로토콜, 프롬프트, 구조화 응답 실행 로직을 정의합니다.
- `notion_auto_organizer/ai/schema.py`: agent 응답을 고정하는 Pydantic 블록 스키마(`OrganizedNote`)와 Markdown 렌더러를 정의합니다.
- `notion_auto_organizer/ai/gemini.py`: LangChain Gemini 제공자 구현입니다.
- `notion_auto_organizer/ai/openai.py`: LangChain OpenAI 호환 제공자 구현입니다.
- `notion_auto_organizer/ai/factory.py`: `AI_PROVIDER`를 기준으로 제공자를 선택합니다.
...
## Claude Code 자동 커밋 훅 예외

- 로컬 `.claude/settings.local.json`(gitignore 처리, 저장소에 커밋되지 않음)에 Stop 훅이 설정되어 있으면, Claude Code 턴이 끝날 때마다 `.claude/hooks/auto-commit-push.sh`가 실행됩니다.
- 이 훅은 커밋되지 않은 변경이 있을 때만 `python -m pytest`를 실행하고, 테스트가 통과한 경우에만 `git add -A`, 커밋, `git push origin master`를 자동으로 수행합니다. 테스트가 실패하면 아무 것도 커밋하지 않고 조용히 종료합니다.
- 이 자동 커밋·푸시는 위 "작업 계획과 작업 로그" 절의 계획 승인 절차를 거치지 않습니다. 이는 저장소 소유자가 명시적으로 수용한 예외이며, 해당 훅이 활성화된 로컬 환경에만 적용됩니다. Claude가 직접 수행하는 저장소 변경(파일 편집, 계획 수립 등)에는 계획/로그 절차가 그대로 적용됩니다.
