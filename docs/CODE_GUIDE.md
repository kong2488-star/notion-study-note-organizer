...
**`NOTE_ORGANIZER_PROMPT`** — AI에게 주는 시스템 프롬프트입니다. "한국어 입문자용 개발 학습 노트 편집자" 역할을 부여하고, 출력 형식을 지정합니다:

- `# 제목 / ## 핵심 요약 / ## 개념 정리 / ## 예시 코드` 섹션 구조 사용
- bold 문법(`**`) 사용 금지 (Notion 변환 시 그대로 노출되는 것 방지)
- 원본에서 언어/프레임워크를 추론하고, 이해를 돕는 짧은 예시 코드 추가
